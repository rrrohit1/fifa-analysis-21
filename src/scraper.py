# This code is attritubed to @4m4n5
# Link: https://github.com/4m4n5/fifa18-all-player-statistics/blob/master/2021/crawler.py
...
for id in data["ID"][:20]:
    ...
    height = (
        meta_data[length - 2].split("'")[0]
        + "'"
        + meta_data[length - 2].split("'")[1].split('"')[0]
    )
    ...
    if "Position" in skill_map.keys():
        if skill_map["Position"] in ("", "RES", "SUB"):
            skill_map["Position"] = (
                soup.find("div", {"class": "meta bp3-text-overflow-ellipsis"})
                .find("span")
                .text
            )
        if skill_map["Position"] != "GK":
            card_rows = soup.find("div", {"class": "lineup"}).find_all(
    ...
    skill_map["Likes"] = (
        columns12[3]
        .find("button", {"class": "bp3-button like-btn need-sign-in"})
        .find("span", {"class": "count"})
        .text
    )
    skill_map["Dislikes"] = (
        columns12[3]
        .find("button", {"class": "bp3-button dislike-btn need-sign-in"})
        .find("span", {"class": "count"})
        .text
    )
    skill_map["Following"] = (
        columns12[3]
        .find("button", {"rel": "nofollow"})
        .find("span", {"class": "count"})
        .text
    )
    name = []
...
full_data.to_csv("data/raw.csv", encoding="utf-8-sig")
